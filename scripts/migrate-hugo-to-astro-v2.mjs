...
/**
 * Hugo to Astro Content Migration Script (v2)
 * 
  ...
  const processedContent = content.replace(wikiLinkRegex, (match, target, displayText) => {
    ...
    for (const [slug, data] of slugMap.entries()) {
      if (data.title === target || 
          ...
          slug === targetSlug) {
        foundSlug = data.hugoSlug || data.slug;
        break;
      }
    }
    ...
    if (foundSlug) {
      ...
      return `[${linkText}](/posts/${foundSlug})`;
    }
    
    // If not found, keep as plain text or link to search
    return `[${linkText}](/posts/?search=${encodeURIComponent(target)})`;
  });
...
async function collectSlugs(files) {
  ...
  for (const file of files) {
    ...
    // Hugo-style slug: YYYY/MM/DD/slug
    const hugoSlug = `${year}/${month}/${day}/${slug}`;
...
async function migrateContent() {
  logger.info('Starting Hugo to Astro migration (v2 - Hugo URL format)...');
  ...
  for (const [slug, slugData] of slugMap.entries()) {
    try {
      ...
      const month = String(date.getMonth() + 1).padStart(2, '0');
      const day = String(date.getDate()).padStart(2, '0');
      const newFileName = `${year}-${month}-${day}-${slug}.md`;
      ...
      const newFrontmatter = {
        ...
        customSlug: slug,  // Store just the slug part
        tags: data.tags || [],
      };
      ...
      // Store URL mapping
      const oldUrl = `/${hugoSlug}/`;
      const newUrl = `/posts/${hugoSlug}/`;
  ...
  fs.writeFileSync(CONFIG.urlMappingFile, JSON.stringify({
    generated: new Date().toISOString(),
  ...
  logger.info(`URL format preserved: /YYYY/MM/DD/slug/`);
}
