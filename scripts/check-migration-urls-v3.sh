...
while IFS= read -r url; do
    ...
    # 在 url-mapping.json 中查找映射
    # 使用 Node.js 处理 JSON 和 URL 匹配
    result=$(node << EOF
const fs = require('fs');
const mapping = JSON.parse(fs.readFileSync('$URL_MAPPING_FILE', 'utf8'));
const path = '$path';

// Try the path as-is (may be URL encoded)
if (mapping.mappings[path]) {
    console.log('MAPPED|' + mapping.mappings[path]);
    process.exit(0);
}
...
try {
    ...
    if (mapping.mappings[decoded]) {
        console.log('MAPPED|' + mapping.mappings[decoded]);
        process.exit(0);
    }
} catch (e) {}

console.log('NOT_MAPPED');
EOF
)

    result_type=$(echo "$result" | cut -d'|' -f1)
    mapped_path=$(echo "$result" | cut -d'|' -f2)
    
    if [ "$result_type" = "MAPPED" ]; then
        # 有映射，检查映射后的 URL
