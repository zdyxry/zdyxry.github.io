...
/**
 * Hugo to Astro Content Migration Script (v5)
 * 
 * Preserves Hugo URL format: /YYYY/MM/DD/slug/
 * Wiki links: [[Keyword]] → [Keyword](/mentions/keyword)
 */
...
function processWikiLinks(content) {
  // Match [[link]] or [[link|text]] - but not inside code blocks
  ...
  for (let i = 0; i < parts.length; i++) {
    // Only process even indices (outside code blocks)
    if (i % 2 === 0) {
      parts[i] = parts[i].replace(wikiLinkRegex, (match, target, displayText) => {
        ...
        // Link to mentions page: /mentions/keyword
        return `[${linkText}](/mentions/${normalizedKeyword})`;
      });
    }
  }
...
async function migrateContent() {
  logger.info('Starting Hugo to Astro migration (v5 - Mentions links)...');
  ...
  logger.info(`Mentions links: [[Keyword]] → /mentions/keyword`);
}
