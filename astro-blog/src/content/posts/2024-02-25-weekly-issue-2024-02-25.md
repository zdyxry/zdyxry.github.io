...
作者在 [CNCF](/mentions/cncf) TOC  两年回顾。
...
继续 [MTU](/mentions/mtu) 系列。使用 iptables 进行丢包验证，注意需要把 GRO(Generic Receive Offload）关掉。不同的 PMTUD 的实现行为不一样。
[ICMP](/mentions/icmp) type 3 表示目标不可达，其中 code 4 表示包太大，需要拆分。但实际上在公网场景下可能不生效。
...
[Linkerd](/mentions/linkerd) 发布 2.15 版本，其中需要注意的是在发布形式上发生了重要变化：**从 2.15 开始，我们将不再生成开源稳定版本。**

[Linkerd](/mentions/linkerd) 自身是以开源项目来管理，但是不会提供构建支持，构建产物最终以 BEL(Buoyant Enterprise for Linkerd) 发布，需要注意 BEL 的免费使用限制：非生产环境。如果员工人数少于 50 人可以免费在生产环境中使用。
...
需要在 CI 环境中运行 [kubernetes](/mentions/kubernetes) 的话，先明确自己需要什么隔离需求，如果需求只需要一个 [kubernetes](/mentions/kubernetes) 环境的话，那么 [k3d](/mentions/k3d) 是最快的。

Kubernetes 自身的 CI 是用 Kind 跑的。我自己本地如果需要 k8s 环境的话，通常是直接在本地部署一个 k3s 了，原来现在 [minikube](/mentions/minikube) 也提供了 Docker 作为 Driver。
...
虚构的有趣的裁员[故事](/posts/故事)，后续也许会成为现实的。
...
[Dropbox](/mentions/dropbox) 员工爆料：自己老板通过阻塞其他组的工作进度，来展示出自己组的状态比较健康，包括不限于：给大量 comments、延迟 review、拒绝 PR、在其他人开发过程中进行大范围的重构，来使对方返工。。。
