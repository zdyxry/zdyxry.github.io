...
用 [Unix](/mentions/unix) 命令风格来写 [Golang](/mentions/golang)，比如 `script.Args().Concat().Match("Error").First(10).Stdout()`
...
[OpenTF](/mentions/opentf) 已经确认，即将 fork [terraform](/mentions/terraform) 来保证开源协议的可用性。期待故事后续的发展。有没有可能像 [RHEL](/mentions/rhel) 和 [centos](/mentions/centos) 一样，因为开源版本，社区更加繁荣？
...
配置个人邮箱域名，使用 [CloudFlare](/mentions/cloudflare)的 Email Routing 来接受邮件，使用 [AWS](/mentions/aws) SES 设置发送邮件，包含了 DKIM 验证（可以防止邮件进入垃圾箱）。
...
作者用 [ChatGPT](/mentions/chatgpt) 记录日记一周，感受很好，这里是详细的 prompt。

我觉得直接发给 [ChatGPT](/mentions/chatgpt) 在国内的环境下还是不太方便，可能需要用其他的形式来中转一下，定期投喂，可能比较好。
