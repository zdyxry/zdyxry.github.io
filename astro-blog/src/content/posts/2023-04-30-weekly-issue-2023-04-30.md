...
关于 [QPS](/mentions/qps) 的计算方式，最大 QPS，最佳线程数。单线程𝑄𝑃𝑆=1000𝑚𝑠/𝑅𝑇。最佳线程数 = (RT/CPU Time) x CPU 核数 x CPU利用率 。
...
貌似很多站点都会检测 [Golang](/mentions/golang) 的 Cipher Suites，之前应该是[v2ray](/mentions/v2ray) 提到过类似问题。通过使用 [GitHub - refraction-networking/utls: Fork of the Go standard TLS library, providing low-level access to the ClientHello for mimicry purposes.](https://github.com/refraction-networking/utls) 可以进行更改。
...
[Golang](/mentions/golang) Channel 关闭的几种场景。
...
使用 [Buildkit](/mentions/buildkit) 来交叉编译项目示例。现在不少项目的 Dockerfile 还非常的原始，可以利用这些很成熟的方式来提升效率。

---
[使用 BuildKit on Kubernetes 构建多架构容器镜像 | Reimu's blog](https://blog.k8s.li/buildkit-on-kubernetes.html)

在 [kubernetes](/mentions/kubernetes)上部署都架构[Buildkit](/mentions/buildkit) 说明。公司内部 CI 已经采用这种方式进行使用了。

---

[Should Rye Exist? · mitsuhiko/rye · Discussion #6 · GitHub](https://github.com/mitsuhiko/rye/discussions/6)

[flask](/mentions/flask) 作者个人项目，来处理 [Python](/mentions/python) 的项目管理问题。 [PDM](/mentions/pdm) 的作者写过一系列的博客用于讲解类似的问题，但是官方社区推进速度非常缓慢。
...
[GDB](/mentions/gdb)及[ptrace](/mentions/ptrace) 基本流程介绍。
...
[CPU](/mentions/cpu)利用率：CPU 未运行空闲线程的时间。CPU利用率很够，很有可能是因为存储介质导致的。应该关注 IPC 指标，它显示了我们在每个 CPU 时钟周期平均完成了多少指令。越高越好。
...
[microud](https://github.com/microud) 同学最近给内部的虚拟化产品编写了 [Packer](/mentions/packer) 插件，我前阵子也在这个基础上增加了多磁盘的支持。整体上 [Packer](/mentions/packer) 的架构逻辑很清晰，感觉重点是如何衡量暴露给用户的配置方式，哪些属性应该暴露出去，以什么形式暴露。
