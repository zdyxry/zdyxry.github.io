...
一个简单[跳板机](/posts/跳板机) 实现：启动一个 sshd server，来监听连接请求（使用的 [GitHub - gliderlabs/ssh: Easy SSH servers in Golang](https://github.com/gliderlabs/ssh) ），默认会进入到一个 TUI 的交互界面（使用 [GitHub - manifoldco/promptui: Interactive prompt for command-line applications](https://github.com/manifoldco/promptui) ），让用户进行用户选择、服务器选择、连接等等。

额外解析了 `scp` 命令，如果检测到 ssh cmd 是 scp 时，会采用自身实现的 scp 来上传/下载文件。

配合阅读： [How the SCP protocol works (Jan Pechanec's weblog)](https://web.archive.org/web/20170215184048/https://blogs.oracle.com/janp/entry/how_the_scp_protocol_works)， 讲解了 [SCP](/mentions/scp) 协议的工作模式，以及具体的示例。
...
用户使用 [Netlify](/mentions/netlify) 免费套餐，但是受到了 [DDOS](/mentions/ddos) 导致要被收取大量费用，[Netlify](/mentions/netlify) 没有账单限制。

吓得我赶紧去看了看自己的 [Vercel](/mentions/vercel) ，看上去还在合理的使用范围内，也没有绑定信用卡，应该还好。

[ServerlessHorrors | Home](https://serverlesshorrors.com/) 收集了很多 [Serverless](/mentions/serverless) 恐怖故事。。。
...
[OCI](/mentions/oci) Image 构建，大部分还是依赖于 `RUN` 命令来实现， 的，但是 `RUN` 命令会是的多架构场景变得困难，且不是 Reproducibility 的。
...
[chainguard.dev](/mentions/chainguarddev) 开发了 [apko](/mentions/apko)， 一个基于 apk 包管理来构建 Image 的工具，用户编辑 apko file 来定义 Image 内容，因为基于包管理器，Image 的所有文件都来自于 apk，可追踪和验证，提供的 Image 也是 Reproducibility 的。（这带来了引申问题，就是如果构建 apk 包？同步开发了 [GitHub - chainguard-dev/melange: build APKs from source code](https://github.com/chainguard-dev/melange) 用来快速构建 apk。）
...
疑问：
- 这里重点好像是 [SBOM](/mentions/sbom) 和 [cosign](/mentions/cosign) 配合？ [cosign](/mentions/cosign) 和[SBOM](/mentions/sbom) 解决的是什么问题？
- 为什么 [chainguard.dev](/mentions/chainguarddev) 没有采用 [RPM](/mentions/rpm) 或者 [DEB](/mentions/deb) 呢？
...
[Flask](/mentions/flask) 作者对 [奥地利](/posts/奥地利) 的简单介绍。读完就是一个词：保守。做什么事情都需要许可，并且有些许可不容易获得。
