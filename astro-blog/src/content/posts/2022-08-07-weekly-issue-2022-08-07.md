...
date: "2022-08-07T15:59:00.000Z"
slug: "Weekly-Issue-2022-08-07"
tags:
  - Weekly
...
[Equinix](/mentions/equinix) 如何进行发行版的快速验证工作的介绍。之前一直是手动测试验证(无法想象)，现在通过 [BuildKite](/mentions/buildkite) ，通过 DSL 来描述 OS 内容，使用 [virt-customize](/mentions/virt-customize) 来进行配置改动，改动完整后，推送到 S3，进行自动化测试，最终推送给客户。

采用 [Buildkite](/mentions/buildkite) 的原因：
...
[BuildKite](/mentions/buildkite) 学习成本不低。
...
关于 [kubernetes](/mentions/kubernetes) 安全配置的一些方法(感觉大部分人都用不上)
...
使用 [1Password](/mentions/1password) 配合 [DNSControl](https://stackexchange.github.io/dnscontrol/) 管理域名

---

[Using Firecracker and Go to run short-lived, untrusted code execution jobs](https://stanislas.blog/2021/08/firecracker/)

使用 [Firecracker](/mentions/firecracker) 来运行一些短生命周期的不可信任务。预先置备了VM pool，通过在 VM 中运行 Agent 来接收 Job，Job 执行完成后 VM 销毁，会自动置备新的 VM 加入到 VM pool 中。

---

[libyear](https://libyear.com/)

我们经常面对维护的项目依赖老旧的情况，平时很少去主动的更新依赖，当等到不得不更新的时候，通常要投入的精力非常大。其中我觉得一个比较重要的原因是我们无法评估当前项目的依赖管理是否处于一个相对”健康“ 的状态， [libyear](/mentions/libyear) 提出通过一种可以量化的数字来检查当前项目的依赖情况，通过比较项目中每一个依赖当前版本与最新版本的发布时间差值之和，来评估项目状态。
...
关于 [ICMP](/mentions/icmp)， [Ping](/mentions/ping)， [Traceroute](/mentions/traceroute) 的实验和解释。
...
关于[声明式](/posts/声明式) 和 [命令式](/posts/命令式) 的讨论。**命令式有[控制流](https://en.wikipedia.org/wiki/Control_flow)，而声明式没有。**
...
**总结起来，[有氧运动](/posts/有氧运动)对肌肉的抑制，除了总热量因素外，还应该补充的几个层面是：**
