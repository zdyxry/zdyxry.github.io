...
[python](/mentions/python) 处理 Excel 方式对比，Calamine 最快，但是只读不能写，pandas/Tablib 相对慢一些，但是支持多种格式。感觉之后如果简单的数据分析需求，还是应该考虑 [DuckDB](/mentions/duckdb)。
...
[Azure](/mentions/azure) 费用计算出现 Bug 导致多扣钱，反复交涉得到退回（如果是我可能交涉几次就放弃了吧。。）
...
[openssh](/mentions/openssh) 常用的安全配置，其中限制 SSH key 登录执行命令的方式学到了：
