...
[Flask](/mentions/flask) 实现[OAuth](/mentions/oauth) 示例，介绍了如何配置 Provider，如何设置 callback url ，如何使用 authorization code 获取 access token 流程。代码很清理，不到 200行，很适合了解 [OAuth](/mentions/oauth) 。
...
[kubernetes](/mentions/kubernetes) 中 [DNS](/mentions/dns) 解析的优先级，可以直接硬编码 FQDN 来减少无效解析。

---

[Nginx Buffer 机制引发的下载故障 - poslua | ms2008 Blog](https://ms2008.github.io/2021/04/08/nginx-buffer/)

[Nginx](/mentions/nginx) 请求响应太大，超过`proxy_max_temp_file_size` 引发的连接超时现象。官方 Ticket 链接： [#1472 (Downloads stop after 1GB depending of network) – nginx](https://trac.nginx.org/nginx/ticket/1472)  
...
日常不会使用，但是可以了解的 [ip](/mentions/ip) 地址小技巧。
...
[apko](/mentions/apko) 的推广文章，主要介绍了目前非主流的集中构建[image](/mentions/image)工具，[apko](/mentions/apko) 所有的文件来源均是 [apk](/mentions/apk) ，这样可以保证文件是可追溯的，类似于Linux 发行版中所有的文件均来自于[RPM](/mentions/rpm)或 [dpkg](/mentions/dpkg)。同时[chainguard.dev](/mentions/chainguarddev) 提供了[melange](/mentions/melange) 用于帮助用户快速的将自己的项目构建[apk](/mentions/apk)，至少理论上可以极大的降低用户成本。
...
将定时任务从[crond](/mentions/crond) 切换到 [kubernetes](/mentions/kubernetes) [cronjob](/mentions/cronjob) ，可以灵活的分配资源，但是带来了很多不确定性，由于[cronjob](/mentions/cronjob) 自身机制（Reconcile、scheduler、sidecard）的启动延迟，监控指标的缺失导致开发人员无法及时获取自己的定时任务是否失败，为什么失败，后续如何手动触发等等。第二篇博客中提到了解决方式，其中关于定时任务的一些监控指标很有用，可以比较清晰的反应定时任务的状态。

### 生活

[影响力的利弊 -#36 - GeekPlux](https://geekplux.com/newsletters/36)
	想到了[谐星聊天会](/posts/谐星聊天会) 的威哥，最开始流量和影响力很小，还可以实名，随着影响力越来越大，隐私一点点的消失，在当下言论环境下明显安全会受到威胁，现在直接消失不见。
