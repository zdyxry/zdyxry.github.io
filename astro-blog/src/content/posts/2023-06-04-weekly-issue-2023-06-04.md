...
[prometheus](/mentions/prometheus) 的 `Summary` 类型和 `Histogram` 类型。之前内部实现网络监控功能的时候，使用了 `Histogram` ，Bucket 的划分粒度比较重要，如果划分的不好那么结果的偏差可能会比较大。
...
[kubernetes](/mentions/kubernetes) 节点时钟不一致导致的[JWT](/mentions/jwt) 校验不通过问题。
...
最近一个同事突然脚痛，无法步行上班的那种，去医院也查不出来原因，最后推给了[痛风](/posts/痛风)。就想起了[阿毛](https://blog.winkidney.com/) 的这篇博客，免疫系统疾病太可怕了，希望大家都能身体健康。
