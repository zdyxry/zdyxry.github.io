...
[GraphDatabase](/mentions/graphdatabase) 的简单介绍，其中提到了一些开源的 [Graph](/mentions/graph) 分析工具：NetworkX, Cytoscape, Gephi等。

---

[简单聊一下开源 - (ゝω·)~ kira](https://blog.dreamfever.me/2023/09/12/jian-dan-liao-yi-xia-kai-yuan/)

这篇文章记录了作者参与 [Bun](/mentions/bun) 的一些感受，背后是有一个公司去运营的，在 Issue 和 PR的管理上都是混乱的。
...
[WSL](/mentions/wsl) 终于可以回收内存了。当设置 `AutoMemoryReclaim` 为 `gradual` 时，当 WSL 闲置 5min 后，自动释放内存。通过设置 `cgroup.memory.reclaim` 来实现，需要禁用 [cgroup/v1](/mentions/cgroupv1) ，当前会导致 [Docker/Desktop](/mentions/dockerdesktop) 无法工作。
...
使用 [Ziglang](/mentions/ziglang) 可以非常容易的实现 Go 程序在不同平台间的编译,不需要处理 cgo 的各种问题。
...
作者将[开发机](/posts/开发机)从 [Manjaro](/mentions/manjaro)切换到了[EndeavourOS](/mentions/endeavouros)。我现在在使用 [Garuda](/mentions/garuda) ，与 [EndeavourOS](/mentions/endeavouros) 的主要区别是：[Garuda](/mentions/garuda) 针对图形界面更友好；默认使用 [btrfs](/mentions/btrfs)创建快照；也意味着更臃肿。
...
之前一直购买的淘宝店铺自从升级为天猫店铺之后，衣服越来越贵了，[1688](/mentions/1688) 可以很方便的找到一些平替产品。
