...
尽量避免使用[Shell](/mentions/shell) 来实现某些功能，虽然在 demo or 早起阶段，[Shell](/mentions/shell) 可以非常快速的实现一些功能，但是随着时间的推移，它会变得非常庞大，且无法测试，迟早需要重写它，重写的越晚，成本就越高。
...
[crates.io](/mentions/cratesio) [事故分析](/posts/事故分析)，一个错误的 PR 导致 13min 宕机，其中分析为什么的阶段，连续的 Why 的答案也很无奈，大家都一样（此处不能用“草台班子”）：没有单元测试、没有 reviewer、没有测试环境、全手动测试。
...
使用 hadolint 遇到的常见 [Dockerfile](/mentions/dockerfile) lint 问题，其中大部分都是包管理器的使用问题，什么时候应该使用 cache，什么时候不应该使用 cache。其中 DL3027 推荐使用 `apt-get/apt-cache` 替代 `apt` ，理由是 `apt` 是面向最终用户的接口，可能会发生变化，前者更稳定。
...
使用[Bluefin](/mentions/bluefin) 配合 [Fleek](/mentions/fleek) 来管理自己的开发机器，其中提到的 `~` 代替 `/` 是 [ostree](/mentions/ostree) 场景下不得不的事情。（上周尝试了 [Bluefin](/mentions/bluefin) 还有些问题，也许应该自己基于 [FCOS](/mentions/fcos) 来维护一个 [ostree](/mentions/ostree)。
...
通过自定义[git](/mentions/git) `diff` ，来显示 [SQLite](/mentions/sqlite) 文本化差异。
...
[Finch](/mentions/finch) 1.0 GA ，支持使用 Seekable OCI [SOCI](/mentions/soci) 来通过延迟加载容器 image 加速启动。
...
[TUIC](/mentions/tuic) 作者的开源总结文，感觉做相关的开源项目，要尽量的避免自己受到一些评论的影响才可以好受些，太难了。
...
解释了很多 [git](/mentions/git) 术语，感觉常用的操作很少，偶尔需要一些特殊操作现查就可以了。 reflog 很重要。

---

[一篇精彩的推文，一年后的推特工程实践。 - Xiaowen.Z Deployed](https://xiaowenz.com/blog/2023/10/twitter-engineering-after-a-year/)

[Twitter](/mentions/twitter) 最近一年的变化比之前几年都大，还得等等看后续，现在还太早了。
...
读的半知半解，没有完全理解作者的意图，一个好的 [PM](/mentions/pm) 对于产品功能上的帮助是非常大的，无论是方向性还是细节处理，思考维度是很好的补充。
