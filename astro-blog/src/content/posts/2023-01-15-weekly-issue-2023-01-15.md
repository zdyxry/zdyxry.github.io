...
[Equinix](/mentions/equinix) 通过 [cloud-init](/mentions/cloud-init) 配合 [kubeadm](/mentions/kubeadm) 部署 [kubernetes](/mentions/kubernetes) 集群。
...
[bufbuild](/mentions/bufbuild) 的 [grpc](/mentions/grpc) 命令行。

---

[Scaling Kubernetes to Thousands of CRDs](https://blog.upbound.io/scaling-kubernetes-to-thousands-of-crds/)   

[Crossplane](/mentions/crossplane) 管理上千个 [CRD](/mentions/crd) 遇到的问题和改进方式。（现在好像一个 [kubernetes](/mentions/kubernetes) 集群里面的 CRD 上百也不稀奇。

---

[简洁的 Bash Programming 技巧 - 团子的小窝](https://kodango.com/simple-bash-programming-skills)   

[Bash](/mentions/bash) 中 `--` 后面的参数不会被当做选项解析。

---

[How Docker 2.0 went from $11M to $135M in 2 years](https://sacra.com/p/docker-plg-pivot/)    

[docker](/mentions/docker) 公司靠着[Dockerhub](/mentions/dockerhub) 和 [Desktop](/mentions/desktop) 收费实现收益。（但现在替代品已经很多了
...
列举了一些常见的 [Golang](/mentions/golang)并发场景。

---

[Why not DNS?](https://whyk8s.substack.com/p/why-not-dns)   

为什么 [k8s](/mentions/k8s)不直接通过 [DNS](/mentions/dns) 来配置 service 至 pod，而是通过 cluster-ip？避免缓存。

---

[Go runtime vs CFS quota - Vladimir Varankin](https://vladimir.varank.in/notes/2023/01/go-runtime-vs-cfs-quota/)  

[Golang](/mentions/golang) runtime 无法感知 资源限制，如果用默认的 `runtime.GOMAXPROCS` 可能会导致应用更早的触发节流，可以考虑使用 [GitHub - uber-go/automaxprocs: Automatically set GOMAXPROCS to match Linux container CPU quota.](https://github.com/uber-go/automaxprocs) 来自动配置。
...
笔记和写作从需求上看是不同的场景，用 [logseq](/mentions/logseq) 来写一篇文章感觉痛苦，有时候甚至不如一个独立的 markdown 编辑器来的爽快。
