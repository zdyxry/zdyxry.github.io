...
社区成员对于 [uv](/mentions/uv) 有很多担忧，其中最大的问题是 uv 背后的公司 [Astral](/mentions/astral) 是拿了投资的，但是现在 [Astral](/mentions/astral) 还没有明确的商业计划，导致不敢使用。[Astral](/mentions/astral) 只是简单的举例说：可能提供 private package registry 来盈利。例子只是一个例子，还有非常多的不确定性。
...
目前关于 [sidecar](/mentions/sidecar) 和 [sidecarless](/mentions/sidecarless) 的观点总结。
...
[Apple](/mentions/apple) 的 Presto 机器（或者叫系统？）可以通过无线的方式更新还没有拆封的手机，用到了 MagSafe 和 NFC 或者其他的方式，来完成开机、更新、关机一系列动作。
...
为什么 [Github](/mentions/github) 成为了代码托管的赢家？当时主流代码托管服务不支持 Git，Ruby 社区的支持，社交属性。在合适的时间点推出了合适的产品，什么是“合适”，如何找到这个时间点，感觉都是巧合，直觉。

---

[[心得] SUSE面試經驗 - 看板 Soft\_Job - 批踢踢實業坊](https://www.ptt.cc/bbs/Soft_Job/M.1725988955.A.012.html)

[SUSE](/mentions/suse) [Rancher](/mentions/rancher) 面试分配的任务，是 [Longhorn](/mentions/longhorn) 一个停留了两年的 issue，没有什么描述，从零开始设计，且是下一个版本必须要包含的内容。离谱。
...
读取数据会导致 [SSD](/mentions/ssd) 寿命吗？不会有影响，读取不会影响电子状态，写入会。

---
[Reasons I still love the fish shell](https://jvns.ca/blog/2024/09/12/reasons-i--still--love-fish/)

[fish](/mentions/fish) 在易用性上确实很好，开箱即用，我在 [zsh](/mentions/zsh) 使用 `zsh-autosuggestions` 来达到 fish 自动补全历史命令的能力。我不用 fish 的原因是有些 POSIX 语法不兼容，比如 `cmd1 && cmd2` 这类的，不过看上去现在已经有改变。
...
[Ziglang](/mentions/ziglang) 将 `ziglang.org` 从 AWS S3 + CloudFront 迁移到了一台 [Hetzner](/mentions/hetzner) VPS 上。他们的心态真不错，`so be it.`。
...
关于如何在 [Golang](/mentions/golang) 中实现 Enum 的几篇文章，可以使用 [GitHub - zarldev/goenums: Type Safe Enum generator for Go](https://github.com/zarldev/goenums) 或者 [GitHub - abice/go-enum: An enum generator for go](https://github.com/abice/go-enum) 来快速生成。
...
[Apple](/mentions/apple) 对耳鸣的调查结果。一共有 16w 人接受了调查，77.6%的人经历过耳鸣，大约有 15% 的人每天都会经历耳鸣, 55 岁以上的人有 35.8%的人经常耳鸣。缓解耳鸣的方式有：白噪音，自然声音，冥想。
...
[大同](/posts/大同) [游记](/posts/游记)，山西最近挺火热的，加上《黑神话悟空》游戏，又出圈了。
...
极空间的易用性真不错，同事也有好几个买了的。一直在想是否有必要搞一个 [NAS](/mentions/nas)，说服自己不需要的方式就是“你现在还在想这个问题，那就是不需要”。
