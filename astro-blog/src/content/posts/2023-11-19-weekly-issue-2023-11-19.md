...
这个兼容性保证可以称得上伟大的成就。（不知道 [curl](/mentions/curl) 是如何进行测试的。
...
[HTTP](/mentions/http) 优先级变化，HTTP/1 没有优先级，HTTP/2 优先级非常复杂：依赖树 x 256 个优先级 x 独占位 x 动态调整，互相乘起来复杂度飙升。HTTP/3 优先级在 HTTP/2 上简化了许多。
