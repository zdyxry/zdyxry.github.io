...
[Databend](/mentions/databend) 效率工程介绍，使用 SaaS 来完成大部分工作，Self Host Github Runner 尽量使用 [AWS](/mentions/aws) 内部服务，来减免流量费用。
...
[Lyft](/mentions/lyft) 针对 [envoy](/mentions/envoy) 配置方式进行的改进，原有存在的问题：

原有路由配置使用[Jinja](/mentions/jinja) 模版进行维护，无法进行很好的语法检查和验证，现已替换为 [Golang](/mentions/golang)

原有 [envoy](/mentions/envoy) API 在配置平面和控制平面都被引用，当 [envoy](/mentions/envoy) 升级时，都需要考虑兼容性问题，现在只在控制平面引用
...
[cgroup](/mentions/cgroup) 使用 cpuset 来避免 CPU throttling 。

---

[How to remove a stuck namespace](https://blog.ediri.io/how-to-remove-a-stuck-namespace)

[kubernetes](/mentions/kubernetes) 如何删除 namespace，通常是因为 [Finalizers](/mentions/finalizers) 。
...
[hashicorp](/mentions/hashicorp) 关于 Cloud 的调查。multi-cloud 已经是事实，大多数是独立的团队来维护 multi-cloud 资源。其中 94% 认为存在资源浪费的情况。
...
什么时候应该进行[微服务](/posts/微服务) 设计？除非理由非常明确，否则不要这样做。
...
从应用部署的角度来讲解为什么我们需要[容器](/posts/容器) 和 [kubernetes](/mentions/kubernetes)。之前还有从应用发布的角度（binary -> tar -> rpm/deb -> container）来讲解为什么需要[容器](/posts/容器)。
...
这本书是在阅读 [《从零开始的女性主义》](/posts/从零开始的女性主义) 和 [《职场妈妈不下班》](/posts/职场妈妈不下班) 之后的第三本关于女性主义的书，相比前两本，这本书讲述的场景更多，让人产生思考也更多，更系统的描述了男性与女性在日常生活中所面临场景的不同，以及男性拥有了哪些不属于自己的权利。当我们在觉得一件事情理所当然的时候，我们就应该反问自己，为什么。比如为什么大家觉得男性会更能忍受痛苦(从而导致在表述相同痛苦时，女性很难得到同等对待)？为什么上司敬酒大部分女性会迫于“社会规则”压力去敬酒？为什么性别相关的恶性犯罪新闻经常使用被动句？作为男性，当面对这类事情发生时，我可以做些什么？