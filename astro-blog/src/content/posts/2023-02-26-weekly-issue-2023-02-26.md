...
[Nix](/mentions/nix) 注定是一个小众的解决方式。

---

[Tcpdump 从 TCP_option_address 中根据真实 IP 过滤 | 卡瓦邦噶！](https://www.kawabangga.com/posts/4925)

[tcpdump](/mentions/tcpdump) 抓取 [LVS](/mentions/lvs) 转发前的真实 IP 地址。
...
早起通过[Google](/mentions/google)[Sheets](/mentions/sheets) 配合 [Lambda](/mentions/lambda) 和 API Gateway 快速验证产品可行性，后续根据需要来切换到后端服务加数据库形式。
...
最近内部遇到的一个问题，使用[containers/image](<https://github.com/containers/image/blob/10858b2058d8c0c709bb2489e1cb7773d1d09216/storage/storage_dest.go#L191-L195>) 这个库，在进行 layer 写入的时候没有 sync，可能会导致 dirty page 太多进而导致程序触发 [OOM](/mentions/oom)。这个场景在配合 xfs 的时候出现问题，ext4 正常。

---

[You Can Stop Updating Copyright Attribution Years](https://hynek.me/til/copyright-years/)

每次更新代码的时候都要考虑是不是更新[Copyright](/mentions/copyright)，越来越多项目部写具体年份了。
[LinuxFoundation](/mentions/linuxfoundation) 针对[Copyright](/mentions/copyright) 有具体的博客讲解：[Copyright Notices in Open Source Software Projects - Linux Foundation](https://www.linuxfoundation.org/blog/blog/copyright-notices-in-open-source-software-projects)

---

[Design | Parca](https://www.parca.dev/docs/parca-agent-design#cpu-sampling-frequency)

[Parca](/mentions/parca) Agent 关于[CPU](/mentions/cpu) 采样频率设置  
...
[iptables](/mentions/iptables) , [LD_PRELOAD](/mentions/ld-preload), 代码中直接修改。评论中提到其他实现方式：把进程放到独立的 network namespace ，然后配合 iptables + route；或者 iptables --uid-owner 匹配 Mark，将不同的进程用不同的 UID 运行，[Istio](/mentions/istio) 采用了这种方式。
