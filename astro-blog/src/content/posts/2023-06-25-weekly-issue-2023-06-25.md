...
> 如果你要基于 UDP 实现一个协议，就要自己处理超过 [MTU](/mentions/mtu) 的问题。  
...
`shebang` 截断方式的改动导致`NixOS` 无法正常工作。 [Perl](/mentions/perl) 解释器会重新读取 `shebang` 。

虽然 [kernel](/mentions/kernel) 没有保证过这个“功能” 是可用的，但是历史原因一直依赖它，并且之前是可以工作的，现在不可以工作，所以这作为一个功能回退，最终被 Linus revert 了。
...
[kernel](/mentions/kernel) [cgroup](/mentions/cgroup) v2 文档中关于这个功能的介绍：
...
[RedHat](/mentions/redhat) 不再公开自己的 [RHEL](/mentions/rhel) 源码，仅对自己的付费用户提供。这里是否符合 [GPL](/mentions/gpl) 存疑（大概率是符合的），但是对于 RockyLinux/AlmaLinux 是一个问题，现在的上下游关系是：Fedora -> CentOS Stream -> RHEL -> RockyLinux/AlmaLinux ，现在 RHEL 不提供源码，那么 RockyLinux/AlmaLinux 无法稳定的得到所有 在 RHEL 中修复（RedHat 自己打 Patch） 但 CentOS Stream 没有的修复，大家是否会彻底的抛弃 RedHat 生态(RPM) 生态？
...
[curl](/mentions/curl) 作者对于[NVD](/mentions/nvd) 评分的不满，根本原因是大家对问题的理解不同，可能一个低级别的问题在 NVD ”看来“ 是高级别的，这可能会影响某些软件的声誉。

---

[We have left the cloud](https://world.hey.com/dhh/we-have-left-the-cloud-251760fb)

[37Signals](/mentions/37signals) 经过6个月的时间，将自己的所有应用从公有云迁出，预计每年可以节省 150w 美元。
...
[Google](/mentions/google) [Cloud Run](/mentions/cloud-run) 第二代执行环境从 [gVisor](/mentions/gvisor) 切换到了 [MicroVM](/mentions/microvm) 。家乐福 CIO 说启动时间快了 4倍。
