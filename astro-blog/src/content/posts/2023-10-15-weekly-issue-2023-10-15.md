...
官方下场，手把手教你如何部署一个离线[kubernetes](/mentions/kubernetes) 集群。

---

[2023 年 10 月 4 日 1.1.1.1 查询失败](https://blog.cloudflare.com/zh-cn/1-1-1-1-lookup-failures-on-october-4th-2023-zh-cn/)

[CloudFlare](/mentions/cloudflare) `1.1.1.1` 因为新的资源记录类型 `ZONEMD` 无法解析，导致使用旧版本的记录，最后由于 [DNSSEC](/mentions/dnssec) 签名过期导致故障。 这里在设计时没有考虑增加对应的报警项，导致这个场景出现的时候完全静默。。
...
[Golang](/mentions/golang) 使用泛型实现 Slice 的 `Clone` 方法的介绍。（但是现在还没有真正使用过泛型。
...
[Hysteria](/mentions/hysteria) 通过采用自己实现拥塞控制算法 **Brutal** 来保证网络通信最大速度。

---

[Why HTTP/3 is eating the world | APNIC Blog](https://blog.apnic.net/2023/09/25/why-http-3-is-eating-the-world/)

[HTTP/3](/mentions/http3) 的占比已经到了19% 。文中提到了 [QUIC](/mentions/quic) 与[TCP](/mentions/tcp) 关于 [TLS](/mentions/tls) 的差异。对于 TCP，TLS 仅加密实际的 HTTP 数据。通过 QUIC，TLS 还对 QUIC 协议本身的大部分进行加密。这意味着 TCP 中所有中间件都可见（且可更改）的元数据（例如数据包编号和连接关闭信号）现在仅可供 QUIC 中的客户端和服务器使用。

我们需要的不是 [HTTP/3](/mentions/http3) 而是 [QUIC](/mentions/quic)。 
...
通过记录田野日志，来不断地观察自己。这应该就是 [Flomo](/mentions/flomo) 的理念吧。

---

[旧文，移动端最好的游戏，《Dream Quest》以及其它 · Issue #274 · yihong0618/gitblog · GitHub](https://github.com/yihong0618/gitblog/issues/274)

[rogue like](/mentions/rogue-like) 游戏是有一定的成瘾性的，个人觉得数值设定是其中比较关键的点。国庆期间在玩[吸血鬼幸存者](/posts/吸血鬼幸存者) ，确实是我最近几年以来，短时间内玩的时间最长的游戏了，预期是比较低的，没想到玩了很久，现在也会时不时的玩上一把。
