...
[字节跳动](/posts/字节跳动) 网络安装操作系统方案，采用 LinuxRoot 配合 u-root 来实现。
...
对于 [shell](/mentions/shell) 所能做到的事情，一直是毫不怀疑的，甚至如果日常中有什么需要快速实现的，我也会选择 [shell](/mentions/shell)，但是如果要产品化的话，选择其他编程语言是一个正确的选择。
...
[docker](/mentions/docker) [compose](/mentions/compose) 的 Fragments 能力，可以用来创建复用的配置。示例：
...
[Palark](/mentions/palark) 内部 [kubernetes](/mentions/kubernetes) 集群 [事故分析](/posts/事故分析)。

内部的日志存储系统运行在 [kubernetes](/mentions/kubernetes) 上，依赖于 [S3](/mentions/s3)，[S3](/mentions/s3) 由使用 [Rook](/mentions/rook) 部署的 [Ceph](/mentions/ceph) 集群提供。这套部署模式一致在不同的虚拟化环境和不同的发行版之间进行部署，都没有遇到问题。

[Rook](/mentions/rook) 在部署 Ceph 时，发现 Ceph 没有正常工作，Operator log 中包含大量的超时请求，日志中没有明显的错误，Monitor 只有在某个时刻收到的 TERM 信号。
...
[md5](/mentions/md5) 碰撞的真实例子，是可打印的 ASCII 发生冲突。
...
[Stripe](/mentions/stripe) 的年终总结，处理了 1 万亿美元，相当于全球 GDP 的 1%。
...
[redis](/mentions/redis) 更改了相关开源协议的说明。对于提供 [redis](/mentions/redis) 竞争服务的公司，无法继续使用后续版本的 [redis](/mentions/redis)。

---

[Linux Crisis Tools](https://www.brendangregg.com/blog//2024-03-24/linux-crisis-tools.html)

[Linux](/mentions/linux) debug 必备的工具集：包含基本的 ps, vmstate, lsblk, iostat, tcpdump 等等，还建议包含 bcc-tools 和 bpftrace 相关工具集。
...
[Neuralink](/mentions/neuralink) 脑机接口已经可以玩游戏了，感觉是跨时代的发展。
