...
终于不用再 [Golang](/mentions/golang) for loop 中重新声明变量了。

[μMon: Stupid simple monitoring](https://tomscii.sig7.se/2022/07/uMon-stupid-simple-monitoring)

作者逆潮流的选择了 [RRDtool](/mentions/rrdtool) 来实现自己的监控系统。

吐槽 [prometheus](/mentions/prometheus) & [Grafana](/mentions/grafana)：占据不少磁盘空间的 [TSDB](/mentions/tsdb)、内存资源消耗、配置复杂、网络通信（闲置系统中的噪音）、复杂的前端页面、DSL。
...
关于 [ostree](/mentions/ostree) 的背景介绍，使用 chroot 来隔离不同的操作系统根文件系统，使用硬链接来避免不同操作系统根文件系统之间的文件重复（通过链接到只读文件系统来限制跨更改的影响）。

[ostree](/mentions/ostree) 已经发展这么久了，好像还没有看到大规模的应用或流行的迹象。作为开发使用的操作系统不会选用这个，因为限制太多了，作为生产上，需要确保所有的应用全部容器化，实际运维管理起来也没有那么美好，毕竟除了 rootfs 的变更，系统配置也是需要配合变更的。
...
[SSH](/mentions/ssh) 的一些使用技巧。vim 可以直接编辑 scp 远程文件，学到了。

---

[Go项目开发指南](https://chat.openai.com/share/8ac0aeec-d862-4c55-a466-95c404e0946f)

如何 step-by-step 的使用 [ChatGPT](/mentions/chatgpt) 来开发一个 [Golang](/mentions/golang) 项目：尽可能的描述清楚需求、尽可能的把问题抛出，让 [ChatGPT](/mentions/chatgpt) 来修改、如果发现有些不符合习惯的地方，尽早提出、阶段性的让 [ChatGPT](/mentions/chatgpt) 回忆当前的状态。
...
看到 [赛博朋克 2077](/posts/赛博朋克-2077) 的新闻，搜了搜相关的故事，真好啊，看开了。
...
因为现在是播客的重度用户，而中文播客中有一个很大的类目，是身心灵，大家追求自己，不断的探寻内心。但是我对这个一直不感冒，尝试过，但是不理解，前段时间听了[学霸猫](/posts/学霸猫) 的音频，对于[学霸猫](/posts/学霸猫)的好友：[张潇雨](/posts/张潇雨)，[Steve](/mentions/steve) 都抱有怀疑的态度。

今天看到有人举报[李松蔚](/posts/李松蔚)性侵，看到了这篇文章，我希望大家扒一扒这些”身心导师“，看看到底是不是水货。
