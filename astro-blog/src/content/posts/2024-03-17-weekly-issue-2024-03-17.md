...
[Obsidian](/mentions/obsidian) 的 [Canvas](/mentions/canvas) 格式 spec，[GitHub - obsidianmd/jsoncanvas: An open file format for infinite canvas data.](https://github.com/obsidianmd/jsoncanvas)。

[Obsidian](/mentions/obsidian) 把自己的理念贯彻的比较彻底，虽然它自身不是开源软件，但是它的文件存储始终是文件文件，这点很重要。 File over app。
...
我：现在存在这么多用 [python](/mentions/python) 写 UI 的项目，都是因为前端项目的复杂度太高了。  
...
[Acorn](/mentions/acorn) 是由 [Rancher](/mentions/rancher) 的初创团队创立的公司，包括 CEO 和 CTO，在 2020 年 Rancher 被 SUSE 收购后，2022 年创立。[Acorn](/mentions/acorn) 早期（截止到 2024 年 3 月之前）一直是将重点放在应用部署和管理上，但是最近将重点放在了 [GPTScript](/mentions/gptscript) 上，并且完全放弃了之前的所有项目。
...
[Apple](/mentions/apple) 允许欧盟开发者通过第三方应用商店上架自己的 app，还允许从网站上直接下载应用。
...
[DHH](/mentions/dhh) 暴论：你不应该太脆弱，如果你不想工作，那么请忍耐一下。不亏是资本家。
...
作者列举了在[日本](/posts/日本) [京都](/posts/京都) 生活了半年之后，无法忍受的点：
- 被迫消费降级
