...
date: "2023-01-29T11:00:00.000Z"
slug: "Weekly-Issue-2023-01-29"
tags:
  - Weekly
...
[朱双印](/posts/朱双印) 关于 [iptables](/mentions/iptables) 的基本介绍。
...
来自 [Google](/mentions/google)真实事件，真正的 [SRE](/mentions/sre)，和这种人工作应该很幸福。
...
[Buildkit](/mentions/buildkit) 新特性引发的兼容性问题。     
...
[ESXi](/mentions/esxi) 虚拟机[HA](/mentions/ha)das.reregisterrestartdisabledvms	
...
什么不应该放到 [git](/mentions/git)中，比如 container image。其中提到了配置文件，理由是如果你部署了一个 Deployment，并且配置了 auto-scale group，现在因为流量变化，导致副本数发生变化，是否要同步更新到配置文件中？现在[GitOps](/mentions/gitops) 的用户应该是很多的，搜了一下没有搜到最佳实践是啥。。

---

[谈谈 Mastodon、Fediverse 和 ActivityPub - zu1k](https://zu1k.com/posts/tutorials/p2p/fediverse/)

越来越多的用户抛弃了 [Twitter](/mentions/twitter) 去了 [Mastodon](/mentions/mastodon)。

---

[【Prisma，下一代ORM，不仅仅是ORM（下篇） | Linbudu's Blog](https://linbudu.top/prisma-02)    

介绍 [Prisma](/mentions/prisma) 相关内容，其中的 [github](/mentions/github) 仓库中有不少示例。
...
[Makefile](/mentions/makefile) 就属于那种不喜欢但又到处都有的东西。

---

[一次线上内存使用率异常问题排查](https://fanlv.fun/2022/06/02/golang-pprof-mem/)

从[TCMalloc](/mentions/tcmalloc) 到 [mmap](/mentions/mmap) ，到 [Golang](/mentions/golang) 中的内存分配都有说明，最后判定问题是内存已经 GC，但是没有归还给操作系统(scavenging)。

---

[GraphQL 客户端缓存的正确打开方式 | Riptide](http://www.myriptide.com/smart-cache-cn/)   

[GraphQL](/mentions/graphql) 客户端缓存处理。
