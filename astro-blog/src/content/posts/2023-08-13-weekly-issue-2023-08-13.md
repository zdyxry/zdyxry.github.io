...
每当觉得 [copilot](/mentions/copilot) 不行的时候，就去写一写单元测试，会感激他的。

最近非常喜欢用 [ChatGPT](/mentions/chatgpt) 问一些 `vs` 问题，这个是在使用 [Google](/mentions/google) 搜索时，一直使用的，当要了解一个东西 A 时，我通常看完这个东西自身的基本信息和文档之后，就去搜索 `A vs`，会看到很多同类的产品和对比，然后一篇一篇的看。现在有了 [ChatGPT](/mentions/chatgpt) 之后，都是直接问：`A 和 B 相比，有什么差别，各自有什么优缺点，在什么场景下推荐使用 A？`，方便了许多。

---

[HashiCorp adopts Business Source License](https://www.hashicorp.com/blog/hashicorp-adopts-business-source-license)

[hashicorp](/mentions/hashicorp) 决定将[开源](/posts/开源) [License](/mentions/license) 从Mozilla Public License v2.0 (MPL 2.0)切换到Business Source License (BSL) v1.1。 [Pulumi](/mentions/pulumi) 是可以直接使用 [terraform](/mentions/terraform) provider 的，这些会收到影响么？

[Pulumi](/mentions/pulumi) CEO 说他们尝试贡献给[terraform](/mentions/terraform) ，但是 [hashicorp](/mentions/hashicorp) 不接受这些提交，只能 fork 单独维护。同时 [Spacelift](/mentions/spacelift) 联合创始人保证用户可以继续使用 [Spacelift](/mentions/spacelift)。
