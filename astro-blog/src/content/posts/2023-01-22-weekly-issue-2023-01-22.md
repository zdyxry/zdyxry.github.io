...
[37Signals](/mentions/37signals) 发布了2022 公有云服务的账单，其中关于数据存储和数据相关服务占比达到了70% ，这部分是最难迁移出来了，期待后续。
...
对[Wolf](/mentions/wolf) 项目的介绍，用来应对供应链攻击。
...
[IaC](/mentions/iac) 之后谈论[IfC](/mentions/ifc) ，在IaC 场景下，开发人员通常需要考虑业务应用程序和基础设施两种应用。  
[IfC](/mentions/ifc) 通过分析业务应用代码来推断所需的基础资源，然后自动创建并维护，不需要手动指定。目前已有的方向是通过 SDK、通过代码注释、两者结合、通过新的编程语言实现。预测 [IfC](/mentions/ifc) 会在之后愈发流行。
[IfC](/mentions/ifc) 创建的资源如何管理？如何保证应用程序没有滥用资源？
...
关于 [Golang](/mentions/golang) [interface](/mentions/interface) 使用上的总结。
...
关于 [Python](/mentions/python)[package](/mentions/package) 的详细介绍文章，并对比了 [JavaScript](/mentions/javascript) 和 [.NET](/mentions/net) 生态，来吐槽 Python 的一团糟。

---

[烧钱工作法 —— 远程开发是怎样提升我的效率？| Oilbeater 的自习室](https://oilbeater.com/2023/01/15/how-remote-coding-improve-productivity/)   
通过 [Jetbrains](/mentions/jetbrains) [gateway](/mentions/gateway) 配置 [GCP](/mentions/gcp) 的 Spot 实例进行远程开发，因为 GCP 是实时计费，所以作者会（被迫）集中注意力到当前工作。
...
使用 [Bare Metal](/mentions/bare-metal) 管理 [kubernetes](/mentions/kubernetes) 集群的回顾。作者选择[kubespray](/mentions/kubespray) 来部署集群，在早期使用感受是较好的，但是随着集群规模越来越大，[kubespray](/mentions/kubespray) 耗时随着节点数成正比，[kubespray](/mentions/kubespray) 带来的灵活性超出了作者想要的预期，带来的维护性翻倍，作者采用[kubeadm](/mentions/kubeadm) 来重新实现自己所需的 ansible playbook。  
在管理过程中，随着对测试集群的要求，作者对各个公有云的 [kubernetes](/mentions/kubernetes)服务置备方式进行了调研，最终目的是想要拉起置备 [kubernetes](/mentions/kubernetes) 集群的操作，保证环境一致。
...
[VictoriaMetrics](/mentions/victoriametrics)如何进行监控数据 [benchmark](/mentions/benchmark)。数据集并不是完全随机的，博客中使用温度和电压举例。最后直接使用[node-exporter](/mentions/node-exporter) 作为数据源，通过部署多个副本来提供实时数据。通过 [VictoriaMetrics](/mentions/victoriametrics)的一些优化配置，最后可以达到 2GB/s 的数据写入速度。
...
最近[暴雪](/posts/暴雪)和[网易](/posts/网易)合作到期，国服要关闭了，我玩暴雪的游戏不多，炉石传说无论是自己玩还是看直播，都是陪着我从大学到工作。
