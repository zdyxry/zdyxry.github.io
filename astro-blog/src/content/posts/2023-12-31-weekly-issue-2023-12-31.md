...
[NetKit](/mentions/netkit) device 介绍，为了提升 Pod-Pod 之间的传输性能而引入，相较于 veth 的对称结构，NetKit 分为 Primary 和 Peer，可以通过 Primary 端将 eBPF 程序附加到 Peer 端中。
...
[Google](/mentions/google) 教你如何编写包容性[文档](/posts/文档)：
...
[Golang](/mentions/golang) 语义化版本管理背景，当需要发布不兼容的版本时，需要显式的切换 go mod 声明。

---

[家庭 Homelab 升级计划: v2 | Manjusaka](https://www.manjusaka.blog/posts/2023/12/28/how-do-I-build-homelab-part2/#V2-%E5%90%AF%E5%8A%A8)

使用 [BGP](/mentions/bgp) 和 [MetalLB](/mentions/metallb) 来给自己的 [kubernetes](/mentions/kubernetes) 集群暴露服务，为了保证 openwrt 的可用性，引入了 keepalived 来实现 VIP。
...
[日本](/posts/日本) [关西](/posts/关西) 游记，写的非常详细，很多景色通过图片看都很美好。
...
一辆 [Tesla](/mentions/tesla) Model 3 第一年的成本大概在43k$ 。
...
济州岛[游记](/posts/游记)。 用同事的话说，济州岛是最方便的出国旅游的地方了。
