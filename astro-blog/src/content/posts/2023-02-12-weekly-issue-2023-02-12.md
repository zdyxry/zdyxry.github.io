...
[ChatGPT](/mentions/chatgpt) 大大降低了大家的使用门槛，只要你有想法，就可以快速POC。

---

[Cloud Backup Best Practices for SMBs](https://www.digitalocean.com/blog/cloud-backup-best-practices-startups-smbs)

云应用的[备份](/posts/备份)实践，借机推广[Snapshooter](/mentions/snapshooter)。
...
> 最后我想说，无论是设计工具还是[前端](/posts/前端)代码，都只是实现创意的工具。最终能够限制创意的，只有不够开放和好奇的大脑。
...
[证书](/posts/证书)管理是个非常麻烦的事情，[openssl](/mentions/openssl) 的命令行也是用过的命令工具里面比较难用的几个之一了。
...
[Supabase](/mentions/supabase) 在文档网站提供[ChatGPT](/mentions/chatgpt) 功能，来辅助用户搜索。如何构建：将完整文档拆分为多份，使用 [OpenAI](/mentions/openai) API 将其作为输入，使用[pgvector](/mentions/pgvector) 存储到[Postgres](/mentions/postgres) 中，获取用户问题，在[Postgres](/mentions/postgres) 中获取问题相关的文档，讲这些文档作为[GPT-3](/mentions/gpt-3) 上下文，返回给用户。
...
[Seccomp](/mentions/seccomp) 对 [redis](/mentions/redis) benchmark 的影响。

---

[Profiling 101 - XX’s Blog](https://xxchan.github.io/cs/2023/02/08/profiling-101.html)

[Profiling](/mentions/profiling)  101。 对于如何在[Python](/mentions/python) 环境中进行有效的性能排查一直没有掌握。
...
[Linux](/mentions/linux)[开发机](/posts/开发机) 配置，选择的发行版是 [Garuda](/mentions/garuda)。 感觉目前因为[企业微信](/posts/企业微信)[微信](/posts/微信)[腾讯会议](/posts/腾讯会议) 等等类似的软件，一台 PC 的情况下还是没办法只使用 [Linux](/mentions/linux)。打算等我的 PC 到了之后尝试一下 [WSL2](/mentions/wsl2)，看看现状如何。文中提到的云服务器比较贵的事情，最近发现如果只是实验性质的工作，可以考虑[竞价实例](/posts/竞价实例)，价格很友好。

---

[下一次工业革命近在眼前了](https://xargin.com/winter-is-coming/)

最近关于 [AI](/mentions/ai) 的讨论热度太高了，感觉跟之前的[元宇宙](/posts/元宇宙) 不同，后者是虚无缥缈的，前者现在是实实在在有可以让你感到震撼的能力了。
