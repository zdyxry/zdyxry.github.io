...
[Shopify](/mentions/shopify) CEO 给 llama.cpp 项目提了一个 PR，技术型老板可以让他们看到更多信息，技术敏感很重要，可以快速了解当前的趋势。有一个技术型老板应该很开心吧。

---

https://ricardoanderegg.com/posts/makefile-python-project-tricks/

[Python](/mentions/python) 项目中使用 [Makefile](/mentions/makefile) 的一些技巧，如 `venv` 设置，help message 输出，`PYTHONPATH` 设置等。
...
[thoughtworks](/mentions/thoughtworks) 技术雷达，会定期更新，对技术/平台/趋势进行评估。

---

https://almalinux.org/blog/future-of-almalinux/
[AlmaLinux](/mentions/almalinux) 因为 [RHEL](/mentions/rhel) 事件，放弃了与 [RHEL](/mentions/rhel) 1:1 兼容，只保证 ABI 兼容。后续上游采用 CentOS Stream。
...
使用[PostgreSQL](/mentions/postgresql) 配合 [Kine](/mentions/kine) 运行 [kubernetes](/mentions/kubernetes)。其中 PG 通过 systemd 控制，Kine 通过 StaticPod 控制。应该只是看看就好，etcd 目前有了 Kubernetes SIG，开始积极维护中了。Kine 的问题也不少。

---

[Why We Replaced Firecracker with QEMU | Hocus Blog](https://hocus.dev/blog/qemu-vs-firecracker/)

[Hocus](/mentions/hocus) 对比 [Firecracker](/mentions/firecracker) 与 [QEMU](/mentions/qemu)，比最终选择[QEMU](/mentions/qemu)， [Firecracker](/mentions/firecracker) 不支持 [GPU](/mentions/gpu)，磁盘 IO 性能也不如 QEMU。  
其中提到 [Firecracker](/mentions/firecracker) 的 [Ballooning](/mentions/ballooning)  不可用，有点不符合预期。
...
[租房](/posts/租房)经验，其中提到 [小红书](/posts/小红书) 是现在一个不错的方式了。
