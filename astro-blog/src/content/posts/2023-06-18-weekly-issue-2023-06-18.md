...
私钥直接提交到了 [github](/mentions/github)，导致 [ETH](/mentions/eth) 被盗。在开发过程中尽量使用环境变量来获取一些敏感信息，有时候写 demo 代码的时候可能图方便就直接 hardcode 了，但可能带来的影响是很容易忽略的。

也有人推荐使用 [git-secrets](/mentions/git-secrets) 在提交代码之前检查是否存在敏感信息。
...
文章中提到的是单个云服务商的 [SLA](/mentions/sla)，现在很多应用都同时在使用很多的 [PaaS](/mentions/paas) 提供的服务，这个时候很难计算自己应用的[SLA](/mentions/sla) 是多少。
...
[Dropbox](/mentions/dropbox) 如何选择 [IDC](/mentions/idc)。
- 确认需求：
...
[Python](/mentions/python) PEP-703 提出 [NOGIL](https://peps.python.org/pep-0703/) 的版本，在 IO性能上最多有10倍的提升。

现在 [Python](/mentions/python) 的性能问题和包管理器混乱的问题，如果不是生态比较好，应该也很难再增长了吧。
...
[开源](/posts/开源) 社区维护思考，从维护者视角和贡献者视角考虑，感觉每个项目都需要一个类似 Project Manager 的角色。
...
现有的信息呈现方式大部分都是二维的，而[Vision Pro](/mentions/vision-pro) 引入了三维的概念，虽然之前几年的 [VR](/mentions/vr) 已经进行了很多的探索，但是没办法说实质性的成功，文章说是因为交互体验上的缺失，[Vision Pro](/mentions/vision-pro) 从交互体验扔掉了手柄，使用眼睛、收拾和语音的组合。

作为 **大就是好** 的拥趸，还是看好[Vision Pro](/mentions/vision-pro) 的，毕竟这是[苹果](/posts/苹果) 啊。
...
[塞尔达传说](/posts/塞尔达传说) 开放世界的构建基础，虽然没有完整的玩过，但是还是看的津津有味。
