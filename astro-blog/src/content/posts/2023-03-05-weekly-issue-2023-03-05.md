...
[MRSK](/mentions/mrsk) 介绍，通过 DSL 和 [docker](/mentions/docker) 运行 [container](/mentions/container) app。
...
[ChatGPT](/mentions/chatgpt) 开放 [API](/mentions/api) 了，应该会有很多应用为了快速吸引用户而接入。

---

[Introducing the Determinate Nix Installer — Determinate Systems](https://determinate.systems/posts/determinate-nix-installer)

[Nix](/mentions/nix) 的安装器官方使用 [Bash](/mentions/bash) 来编写的，但是 [Bash](/mentions/bash) 对于环境兼容和适配工作是很繁琐的事情，[Determinate Systems](/mentions/determinate-systems) 使用[Rust](/mentions/rust) 重写了整个 安装器，并提供了完整的卸载功能。
对于 [Nix](/mentions/nix) 的安装失败/卸载/重复安装问题，提出了 `receipt` 的概念，用来记录安装过程中涉及到的所有动作：
...
[Dashworks](/mentions/dashworks) 集成在 [Slack](/mentions/slack) 中的 [metasearch](/mentions/metasearch)，可以快速检索分散在各处的信息并分析汇总。我感觉很需要这个，日常信息散落在 Slack/Jira/Github/GitLab/Google Drive.... 真的太烦了。
...
[Nix](/mentions/nix) 101 。完全可以只把[Nix](/mentions/nix) 当做一个包管理器使用，最近在内部项目中使用了 [devbox](/mentions/devbox) 作为环境依赖声明，使用起来很方便。
...
使用[vNUMA](/mentions/vnuma) 配置来创建嵌套的 [ESXi](/mentions/esxi) 节点。

---

[网络问题定位工具记录 - 轩脉刃 - 博客园](https://www.cnblogs.com/yjf512/p/17174669.html)

使用[nstat](/mentions/nstat) 获取网络接口统计信息， `watch -d -n1 "nstat -a | grep -i -E 'drop|error|fail|loss|overflow'"`

---

[Prometheus 数据存储那些事儿 - luozhiyun`s Blog](https://www.luozhiyun.com/archives/725)

[prometheus](/mentions/prometheus) 数据存储介绍。
...
看似是在介绍[Nix](/mentions/nix) build 是如何工作的，但是读下来的感觉就是在劝退。
