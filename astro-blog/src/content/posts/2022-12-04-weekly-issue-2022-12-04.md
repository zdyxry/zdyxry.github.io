...
[docker](/mentions/docker) build 因为 `ulimit` 配置，执行 [yum](/mentions/yum) 很慢。

---

[使用 neovim 作为 PDE(个性化开发环境) - 咸糖 - 自律者自由](https://vim0.com/post/neovim/)   

[neovim](/mentions/neovim) 配置。
...
使用 [github](/mentions/github) [Vercel](/mentions/vercel)[Crowdin](/mentions/crowdin) 工具链来进行文档翻译。

---

[Twitter 实习生 George Hotz |程序员的喵](https://catcoding.me/p/geohot/)   

[George Hotz](/mentions/george-hotz) 的直播很有趣。
...
[Finch](/mentions/finch) 通过组装 [lima](/mentions/lima)[containerd](/mentions/containerd) 和 [nerdctl](/mentions/nerdctl) 来简化用户使用成本。
...
使用 [OpenTelemetry](/mentions/opentelemetry) 来观测 [kubernetes](/mentions/kubernetes)内部状态。   
部分 [OCI](/mentions/oci) 没有计划支持 [OpenTelemetry](/mentions/opentelemetry)，用 [Rust](/mentions/rust) 编写的[youki](/mentions/youki) 有这个计划。

---

[What is runwasi - Nigel Poulton](https://nigelpoulton.com/what-is-runwasi/)   

通过 [runwasi](/mentions/runwasi) 实现 [containerd](/mentions/containerd) shim，来让 [kubernetes](/mentions/kubernetes) 支持运行 [wasm](/mentions/wasm)
...
服务器最多可以创建多少个 [TCP](/mentions/tcp)连接： 在内存、文件句柄足够的话一台服务器上可以创建的TCP连接数量是没有限制的。

---

[Drag and drop from terminal](https://blog.meain.io/2022/terminal-drag-and-drop/)   

从 [terminal](/mentions/terminal)直接拖拽文件
...
[TailScale](/mentions/tailscale) [ACL](/mentions/acl) 配置方式。之前看了官方文档，但是自己的需求比较少，暂时还不需要。

---

[Building A Virtual Machine inside ChatGPT](https://www.engraved.blog/building-a-virtual-machine-inside/)    

[ChatGPT](/mentions/chatgpt) 模拟一台虚拟机，实际上真的是处于模拟状态。

---

[Finding suspicious syscalls with the seccomp notifier | Kubernetes](https://kubernetes.io/blog/2022/12/02/seccomp-notifier/)   

通过 [Seccomp](/mentions/seccomp) Notifier 来追踪 [Container](/mentions/container) 异常系统调用。
...
通过启动新的 [Container](/mentions/container)与目标容器共享[namespace](/mentions/namespace) 的方式来实现端口转发。

---

[一些关于时间和定时任务的库](https://colobu.com/2022/11/26/some-time-and-cron-libs/)   

[Golang](/mentions/golang) 第三方库，时间、[cronjob](/mentions/cronjob) 相关。
