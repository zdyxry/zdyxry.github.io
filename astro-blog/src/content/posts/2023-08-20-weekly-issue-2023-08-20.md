...
[Nginx](/mentions/nginx) 配置 `location` 和 `alias` 配合可能存在安全隐患，需要保证在 `localtion` 和 `alias` 中都以 `/` 结尾。
...
[hashicorp](/mentions/hashicorp) 改变了 [License](/mentions/license) 之后，社区对于 [terraform](/mentions/terraform) 的未来感到担忧，发起了宣言，期望 [hashicorp](/mentions/hashicorp)可以将 [License](/mentions/license)改回 [MPL](/mentions/mpl)，最坏的打算是后续进行 fork 维护。
...
[AWS](/mentions/aws) 的 [Spot](/mentions/spot) 实例价格上涨， [Spot](/mentions/spot) 的低价，是由那些租用了标准实例但是没有100% 使用的人支付的。
...
[Gitlab](/mentions/gitlab) 关于 [DRI](/mentions/dri) 的解释说明。
