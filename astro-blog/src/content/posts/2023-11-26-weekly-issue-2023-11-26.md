...
[Manjusaka](/mentions/manjusaka) 提到的关于 [Aliyun](/mentions/aliyun) `ossutils` 体验很差的吐槽，其中很多是通用的：
...
其中 [ayanamist](/mentions/ayanamist) 的总结是：都是外包做的，而且文档都很差，主要是没有办法给些文档这件事打绩效，做 DX 都是无法打绩效的，如何评判工作做的是否好呢？
...
[Canonical](/mentions/canonical) 推出 [Microcloud](/mentions/microcloud) ，应对小型私有云场景，看上去也挺好的，可以用用。（demo 翻车是常见操作了，还是录制视频比较靠谱。
...
关于[python](/mentions/python) 的错误处理，其中提到了 [GitHub - rustedpy/result: A simple Rust like Result type for Python 3. Fully type annotated.](https://github.com/rustedpy/result) 这个库，可以让[python](/mentions/python) 的返回值变为[Rust](/mentions/rust) 模样。
...
[python](/mentions/python) `datetime.datetime` 的 `utcnow()` 和 `utcfromtimestamp()` 将被移除，需要使用 `datetime.datetime.now(datetime.UTC)` 来替代。
...
很多人第一次编写 [shell](/mentions/shell) 时候，都会遇到 `[` 和 `[` 的问题，这篇文章总结的很好： `[` 与 `test` 是一个东西，`[[` 是 [[Bash](/posts/的问题这篇文章总结的很好-与-test-是一个东西-是-bash) 中的能力。如果想要写通用的 [shell](/mentions/shell) ，那么使用 `[` ，如果明确执行的环境是 [Bash](/mentions/bash)，那么保持使用 `[[` 。 
...
作者使用 3台 [NUC](/mentions/nuc) 来构建自己的 [homelab](/mentions/homelab) ，手把手教学。

---

[How moving from AWS to Bare-Metal saved us $230,000 /yr.](https://blog.oneuptime.com/moving-from-aws-to-bare-metal/)

作者从[AWS](/mentions/aws) 迁回自建机房，本身业务 CICD 都是基于 K8s 的，在 BareMetal 上使用 MicroK8s 来管理 K8s 集群，配合使用 [NFS](/mentions/nfs) 和 [MetalLB](/mentions/metallb) 。
...
通过 [vector](/mentions/vector) 将 [PostgreSQL](/mentions/postgresql) 的异常日志暴露给 [prometheus](/mentions/prometheus)，只需要使用 vector 就可以实现日志的读取、解析、结构化、过滤、转换为 exporter。最终暴露的指标是异常日志的数量，通过检查最近 30min 内日志数量是否有变化来产生报警。
...
介绍 [Windmill](/mentions/windmill) Workflow ，受伤的只有 [airflow](/mentions/airflow) ，[airflow](/mentions/airflow) 真的是太慢了。
...
将[Python](/mentions/python) 程序错误分为4类：Type 错误；import 错误；runtime 错误；业务逻辑错误。如何有效的避免这些错误，应该尽可能的降低错误类别，将业务逻辑错误转换为 runtime 错误，将 runtime 错误转换为 import 错误。 尽可能的提前暴露错误。
...
[Python GIL](/mentions/python-gil) 的行为在不同版本中的变化，其中提到了Python 3.9和Python 3.13之间GIL实现的差异，特别是在释放GIL的时机上的变化。Python 3.13引入了更强的原子性保证，使得大多数字节码序列都是原子的，而Python 3.9则只有很少一部分字节码序列是原子的。
...
是[Faker](/mentions/faker) 让[LOL](/mentions/lol) 更精彩。
...
[Mozilla](/mentions/mozilla) 开源礼节，友好，耐心。共勉。
