...
date: "2022-09-11T15:50:00.000Z"
slug: "Weekly-Issue-2022-09-11"
tags:
  - Weekly
...
如何使用 [Python](/mentions/python) 发送读取 [TUN](/mentions/tun) 设备数据。
...
如何选择基础 [Container](/mentions/container) [image](/mentions/image) 一直是一个比较重要的考虑，比如很多人不推荐的 [alpine](/mentions/alpine)，小而美，但是 [musl](/mentions/musl) 不那么美。很多人推荐使用标准发行版，但是又觉得大小太大不利于分发。 [Distroless](/mentions/distroless) 是一个可以看到的方向，但是我个人在不能完全确定自己的项目中是否依赖了某些隐式的内容时，应该还是会使用标准发行版，比如[Debian](/mentions/debian) 或者 [Fedora](/mentions/fedora)。
...
博主在 [Twitter](/mentions/twitter)  上发起了一些关于 [Container](/mentions/container) [image](/mentions/image) 的投票，一些人表面关注 CVE，实际上还是更关注镜像大小。 [alpine](/mentions/alpine) 是目前大多数项目的选择，但是已经出现了一些替代品。
...
关于 [On-Call](/mentions/on-call) 建立正确的心理预期：
...
[Discord](/mentions/discord) 为了解决 [GCP](/mentions/gcp) 磁盘稳定性的问题，通过 [mdadm](/mentions/mdadm) 来创建 md 设备，将多块 SSD 创建为 RAID0，再将 RAID0 设备和 非 SSD 磁盘创建 RAID1，并配置 `write-mostly` 参数，来让大多数写入到非 SSD 磁盘 。
