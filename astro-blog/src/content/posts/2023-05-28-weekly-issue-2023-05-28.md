...
[Python2](/mentions/python2) 和 [Python3](/mentions/python3)关于 Exception 的处理有很大的不一样，之前学习 `raise` 的差异时看到过类似讨论： [python - Difference between "raise" and "raise e"? - Stack Overflow](https://stackoverflow.com/questions/36153805/difference-between-raise-and-raise-e)
...
非常精彩的[包管理器](/posts/包管理器) 的讨论，往往讨论 [Python](/mentions/python) 的分发方式时，容易跑偏（比如：我用 venv 去打包，为啥不直接用 [Container](/mentions/container)），这篇文章从不同的层面去讨论，讲的很好。

---

[[译] NAT 穿透是如何工作的：技术原理及企业级实践（Tailscale, 2020）](http://arthurchiao.art/blog/how-nat-traversal-works-zh/)

[TailScale](/mentions/tailscale) 关于[NAT](/mentions/nat) 穿透的工作方式介绍，每一个场景逐步解决各个问题。
...
像[Rust](/mentions/rust) 一样编写[Python](/mentions/python)，善用 Type hints，使用 Dataclasses 声明数据类型，使用 `typing.NewType` 来限制参数类型。
...
[Replit](/mentions/replit) 使用 [Nix](/mentions/nix) 维护所有的环境配置，保持一致性。
...
最近用 [Slack](/mentions/slack) 经常提示这个，原来是 [Slack](/mentions/slack) 不推荐不加任何信息的在 thread 直接 @ 人，可以通过 `cc @` 的方式来 @ 某人，这个设计不错，如果单独的 @ 可能被 @ 的人没有任何信息，感觉莫名其妙。学习了。
