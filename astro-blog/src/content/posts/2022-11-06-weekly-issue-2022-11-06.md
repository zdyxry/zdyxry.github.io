...
date: "2022-11-06T15:50:00.000Z"
slug: "Weekly-Issue-2022-11-06"
tags:
  - Weekly
...
[CloudFlare](/mentions/cloudflare) 故障复盘，在引入 opentracing 时清理了部分 header 信息，导致上游逻辑判断错误。

---

[Give Your Tail a Nudge](https://brooker.co.za/blog/2022/10/21/nudge.html)   
[Nudge](/mentions/nudge) 对于 [FCFS](/mentions/fcfs) 的优化：
...
[Python](/mentions/python) 3.11 发布说明提到有不小的性能提升： "Python 3.11 is between 10-60% faster than Python 3.10. On average, we measured a 1.25x speedup on the standard benchmark suite. See [Faster CPython](https://docs.python.org/3/whatsnew/3.11.html#whatsnew311-faster-cpython) for details."，作者针对一些常见场景进行了性能测试对比。
...
通过从 [kubelet](/mentions/kubelet) 直接获取 metric 来展示给用户，轻量的 [kubernetes](/mentions/kubernetes) top.

---

[A Visual Guide to SSH Tunnels (with labs)](https://iximiuz.com/en/posts/ssh-tunnels/)   
[SSH](/mentions/ssh) Tunnel 介绍。(把事情讲明白是一种能力啊)

---


[理解一下依赖注入，以及如何用wire](https://farer.org/2021/04/21/go-dependency-injection-wire/)   
通过 `google/wire` 实现 [依赖注入](/posts/依赖注入)。
...
通过在 [grpc](/mentions/grpc) 中间件将自定义错误转换为对应业务码，同时支持 gRPC 和 HTTP 状态码。
...
使用 [Golang](/mentions/golang) 通过 [goroutine](/mentions/goroutine) 和 [channel](/mentions/channel) 并行处理大文件。

---

[HTTP Resource Leak Mysteries in Go - Coder](https://coder.com/blog/go-leak-mysteries)   
[Golang](/mentions/golang) 内存泄漏调查，原因是没有执行 `response.Body.Close()`
...
[Clay](/mentions/clay) 是面向个人用户的 [CRM](/mentions/crm)。肯能有人觉得有些“功利”，但是人际关系是需要认真维护的。
