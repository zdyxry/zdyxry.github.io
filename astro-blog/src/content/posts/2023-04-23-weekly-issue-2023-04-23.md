...
[TailScale](/mentions/tailscale) 更改了自己的定价策略，在现有用户数据的支持下，解释做出这样变更的原因。免费团队支持3个用户，100个设备。
...
[kubernetes](/mentions/kubernetes) 可以通过 kubelet API 获取 Host 节点上的日志。这个功能是 [openshift](/mentions/openshift) 贡献的。
...
[Gin](/mentions/gin) 介绍，包含了 Engine 和RouterGroup 。
...
[Dataset](/mentions/dataset) 使用暴力来实现日志检索功能。这里的暴力是相对于关键词索引的方式来说。暴力实现是可行的，在适当的场景使用暴力实现是一种权衡，相对来说暴力实现可以提供一致的性能，减少了可能的意外引入，用户感知的性能是整个系统的性能，不仅仅是内部的细节实现。
...
[巴厘岛](/posts/巴厘岛)游记，很真实很有趣。去外地如果吃不惯东西，麦当劳和肯德基是最不会出错的选择。
