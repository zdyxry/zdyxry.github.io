...
作者在 [Gitlab](/mentions/gitlab)工作 6 年多，这篇文章是在保密协议过期之后的回忆，记录了在这几年所做的内容，能感受到期间的挣扎。
作者学到了什么：
- 可扩展性需要成为公司文化的一部分
	- gitlab.com 不是 [Gitlab](/mentions/gitlab) 的主要盈利方式，所以不受重视，gitlab.com 遇到的问题的解决方式通常也无法应用在 self-hosted 版本上。
- 让团队变为数据和开发人员驱动
- 如果没有数据，无法确定什么是“最小可行”
...
[0xide](/mentions/0xide) 的薪资水平说明，所有人都是相同的薪水。
...
通过 [Chromedp](/mentions/chromedp) 来使用 [Golang](/mentions/golang) 操作 [Chrome](/mentions/chrome) 示例，其中一些关键的地方需要使用 [JavaScript](/mentions/javascript) 配合 `chromedp.Evaluate` 实现。[Chromedp](/mentions/chromedp) 是 [Chrome](/mentions/chrome) only 的，所以如果真的需要做一些浏览器自动化的事情，感觉还是使用 [Playwright](/mentions/playwright) 好一些。
...
[Golang](/mentions/golang) 对 `os.File` 对象的单个 `Read` 调用上限是 1 GiB，即使设置 buffer size 是 2GiB，也会一次只读取 1GiB。最早修复是将限制设置为 2GiB-1Bytes，修复 MacOS、FreeBSD 上读取大于等于 2GiB 大小失败问题；随后将限制设置为 1GiB，来保证数据对齐。

---

[uv: Python packaging in Rust](https://astral.sh/blog/uv)

[Astral](/mentions/astral) 公司开发的 [Python](/mentions/python) 工具链，用来替代 [pip](/mentions/pip) ，与 [Ruff](/mentions/ruff) 一样，[uv](/mentions/uv) 专注于性能，在没有缓存的情况快可以比 pip 快 8-10 倍。最终的目标是打造成 [Python](/mentions/python) 的 [cargo](/mentions/cargo)。

 现在 [Rust](/mentions/rust) 对 [JavaScript](/mentions/javascript) 和 [Python](/mentions/python) 的生态改善是明显的，重写一切。

---

[The Three Virtues of a GREAT Programmer](https://thethreevirtues.com/)

[Perl](/mentions/perl) 作者认为的三大美德：懒惰、不耐烦和傲慢。
...
容器内访问 [HTTPS](/mentions/https) 会卡主，访问 [HTTP](/mentions/http) 没问题， Host 访问 [HTTPS](/mentions/https) 和 [http](/mentions/http) 都没问题。
抓包发现 server 443 返回的包 length == 0 ，猜测是包 MTU 太大导致被丢弃。
MTU 异常，是 [TCP](/mentions/tcp) [MSS](/mentions/mss) 协商有问题，于是检查网卡的 MTU 配置，发现 Host 是 1450，Container 内部是 1500。
解决方式：
- [iptables](/mentions/iptables) 设置 TCP 握手的 MSS 值
...
作者建议尽量使用[Golang](/mentions/golang) 标准库来作为初始实现，以 `log/slog` , `net/http`, `database/sql` 举例说明。
...
作者认为 [prometheus](/mentions/prometheus) 的 `irate` 无法正确的捕获峰值数据，因为 `irate` 只计算当前区间的最后两个点，可能会丢失其他峰值数据，具体是否有效取决于 start 和 end 以及 step 数值。

作者建议使用 [VictoriaMetrics](/mentions/victoriametrics) 的 `rollup_rate` 。
...
[TinyPilot](/mentions/tinypilot) 作者的 2023 总结。一人公司的典范。
