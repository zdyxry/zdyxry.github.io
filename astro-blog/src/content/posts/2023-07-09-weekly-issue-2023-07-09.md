...
上周在搜索的时候收到了 [Reddit](/mentions/reddit) 事件的影响，互联网越来越封闭，如何有效的获取信息已经是一个有门槛的事情了。
...
[Golang](/mentions/golang) 跨平台编译的问题以及解决方式。如果一定要用 CGO，还是提供 container 方式吧。

---

https://xeiaso.net/blog/nosleep

避免在[单元测试](/posts/单元测试)中引入 `sleep` ，如果需要需要通知机制，可以使用其他方式，比如 channel。

---

[MPTCP: 一个在 Go 1.21中的被忽略的新特性](https://colobu.com/2023/07/03/mptcp-a-go-1-21-new-feature/)

[Golang](/mentions/golang) 1.21 增加 [MPTCP](/mentions/mptcp) 支持。
> 它允许一个连接同时使用多个网络接口。  
...
通过实际问题了解[TCP](/mentions/tcp) [拥塞控制](/posts/拥塞控制)。 通过人为使用 `ip route` 设置 `initcwnd` 调整。
...
继续 [RedHat](/mentions/redhat) 事件，这篇文章写的很好，把 [centos](/mentions/centos) 背景，不同的产品带来的优势、对 RedHat 的影响都交代的很清楚。如果没有 [centos](/mentions/centos)， [RHEL](/mentions/rhel)也不会有今天这样的影响力。
...
周末的时候因为[Twitter](/mentions/twitter) 搞限流，于是被迫又去使用了[Mastodon](/mentions/mastodon) 了，社交关系的迁移真的是太麻烦了，我先过滤了一波关注列表的 profile 中提到了 Mastodon 的链接，然后通过关注的关注，不断的找到眼熟的，找了很久也不过找到了46个关注的人。

期待 [Meta](/mentions/meta) 的 [Thread](/mentions/thread)。至少 [Meta](/mentions/meta) 不会瞎折腾，他们还是看重用户的。

---

https://stephanango.com/file-over-app

避免 [Vendor lock-in](/mentions/vendor-lock-in) ，文件是永存的，现在使用各种应用会优先考虑格式和导出问题，如果不可导出的，那么就做好随时会遗弃的准备。
...
继续[Twitter](/mentions/twitter) 难民故事，[Meta](/mentions/meta) 新推出的 [Threads](/mentions/threads) 计划支持 [Fediverse](/mentions/fediverse) ，已知的有[Tumblr](/mentions/tumblr) 和 [WordPress](/mentions/wordpress) ，感觉之后会因为个人站点的稳定性不足导致大家去迁移到[Threads](/mentions/threads)，毕竟可以共用 [Instagram](/mentions/instagram) 账号。
