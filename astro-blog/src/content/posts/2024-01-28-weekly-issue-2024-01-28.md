...
[NTP](/mentions/ntp) 协议作者去世了。R.I.P.
...
使用 Ctrl-r 配合 fzf 的使用体验是绝佳的，在自己的开发机上，可以快速地找到自己所有的历史记录。有些命令是低频但是复杂的，记录到笔记上当然可行，但又好像有点没必要。最近越来越觉得保留 Shell 历史记录是一件收益非常高的事情，推荐所有人都使用 [Atuin](/mentions/atuin) 来保存自己的历史记录，或者直接用 Git Repo 来保存也可以。

如果你遇到 [Atuin](/mentions/atuin) 配合 [zsh](/mentions/zsh) 无法记录 `#` 前缀的命令，可以参考这个 Issue 来解决：[Add option to include commands that start with \`#\` in history · Issue #467 · atuinsh/atuin · GitHub](https://github.com/atuinsh/atuin/issues/467)
...
可以通过[Fly.io](/mentions/flyio) 的[DevRel](/mentions/devrel) 的 JD，来感受一下什么是[DevRel](/mentions/devrel) ，以及是做什么的。
...
[开源软件](/posts/开源软件) 的维护者给出的建议。维护开源是一件很容易让人疲惫的事情。如果你的 PR/Issue 没有立即解决，请耐心等待一下；沟通的时候描述的事情尽可能的清晰完整，不要假设对方已经了解完整的上下文。
...
通过在 [Golang](/mentions/golang) 代码仓库中增加 tools.go 文件，在文件中声明依赖的工具，执行 `go mod tidy` 自动拉取相关依赖。[kubernetes](/mentions/kubernetes) 示例：[kubernetes/hack/tools/tools.go at master · kubernetes/kubernetes · GitHub](https://github.com/kubernetes/kubernetes/blob/master/hack/tools/tools.go)
...
[哄哄模拟器](/posts/哄哄模拟器) 作者的复盘。奇怪的流量入口居然是 QQ 群和 QQ 空间，这就是典型的私域流量了吧，真是神奇。最后因为腾讯的屏幕，导致流量急速下滑。
