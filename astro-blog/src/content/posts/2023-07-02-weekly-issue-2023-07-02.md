...
[copilot](/mentions/copilot) 的一些技巧，总结下来就是假象在结对编程，尽可能的细粒度告诉对方当下的想法。
...
关于 [monitor](/mentions/monitor) 的一些思考，包含 Logs，Metrics，Traces 。

日志级别和格式混乱,没有共识。日志被用于各种用途,从调试到业务分析,使其变得极为关键，存储成本高昂,大多数日志从未被使用。

Metrics 很好，但是当前 [prometheus](/mentions/prometheus) 生态在后续的扩展上没有那么容易。
...
[kubernetes](/mentions/kubernetes) [Pod](/mentions/pod) checkpoint 方式，后续应该可以作为 Pod Live Migration 的前置动作。
...
[RedHat](/mentions/redhat) 事情继续发酵，目前官方态度很明确，不会进行改动。这篇文章有些后有趣的描述。比如：
...
[Rocky](/mentions/rocky) Linux 计划从其他方式获取 [RHEL](/mentions/rhel) SRPM： [RedHat](/mentions/redhat) 官方发布的 container image、去付费使用[公有云](/posts/公有云)创建[RHEL](/mentions/rhel)实例。
...
使用 [Cloudflare](/mentions/cloudflare) [Tunnel](/mentions/tunnel) 来对外暴露 [kubernetes](/mentions/kubernetes) 服务。
...
这篇文章写的真好。[朋友](/posts/朋友) 是需要维护的，我很喜欢自己和朋友在一起的状态，但是通常又不会主动的去迈出线下沟通的那一步，有些矛盾。
