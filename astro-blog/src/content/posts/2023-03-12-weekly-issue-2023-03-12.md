...
[MacOS](/mentions/macos) [SIP](/mentions/sip) 严重影响性能。
...
提到了一些 [Prompt](/mentions/prompt) 小技巧。昨天内部同事在 [slack](/mentions/slack) 接入了 [ChatGPT](/mentions/chatgpt) ，在没有合适的 [Prompt](/mentions/prompt) 情况下，几乎是不可用的。
...
作者认为 [Python](/mentions/python) 现在分为 **untyped Python** 和 **typed Python** ，并会永远的持续下去。最近公司的项目在从 [Python2](/mentions/python2) 迁移至[Python3](/mentions/python3) ，如果之后选择 library，那肯定会优先选择 type hints 的。

---

[The pains of GitOps 1.0 | Codefresh](https://codefresh.io/blog/pains-gitops-1-0/)

对于 [GitOps](/mentions/gitops)管理的资源后续因为 auto-scale 等配置引发的不一致情况，[Argo](/mentions/argo) 通过[custom diff](https://github.com/argoproj/argo-cd/issues/2913) 来避免，[terraform](/mentions/terraform) 通过 [ignore_changes](https://developer.hashicorp.com/terraform/language/meta-arguments/lifecycle#ignore_changes) ，来避免。
...
[runc](/mentions/runc) 在 [v1.0.0-rc93](https://bugzilla.redhat.com/show_bug.cgi?id=1934177) 的改动导致的 breaking change。在使用 `distroless/static:nonroot` 时，将用户设置为 `nonroot` UID 是`65532` ，并将工作目录设置为 `/home/nonroot` ，权限是 0700。
之前版本的 [runc](/mentions/runc) 会先 `chdir` 然后设置用户，在执行 `chdir` 的时候会以 `root` 来执行，所以不会因为权限而抛出错误。而 rc93 版本改为先设置用户，再 `chdir`，所以会遇到 permission denied 问题。runc 改动链接： [libctr/init_linux: reorder chdir · opencontainers/runc@d869d05 · GitHub](https://github.com/opencontainers/runc/commit/d869d05aba0fc2519060b3b20b679ac5a5068e78)
这个事情引发的另一个思考是，我理解 [Crossplane](/mentions/crossplane) 提供的产品已经是较为上层了，但是他们的客户遇到问题需要 OnCall 到 OCI 这一层，莫名的为我司 [kubernetes](/mentions/kubernetes) 产品之后的运维成本担忧。
...
我司一体机的型号之一是 [Dell](/mentions/dell) ，因为一些历史原因，所以[DELLBOSS VD](/mentions/dellboss-vd) 只是用来做引导分区，平时完全没有读写，按照这个故障率，我们真的太奢侈了。

---

[垃圾腾讯云CDN：从入门到放弃](https://mp.weixin.qq.com/s/ANFnbDXwuhKI99fgYRZ9ug)

[CDN](/mentions/cdn)预热需要回源请求，但是回源请求产生的流量需要收费，离谱。
